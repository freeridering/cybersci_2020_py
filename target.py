...
class TargetSingle:
    ...
    def init_pos_now(self, tid: int, p: parameter.Parameter):
        ...
        pos_now[0] = flex_axis * (tid // temp_num + 1) - 1
        pos_now[1] = flex_axis * (tid % temp_num + 1) - 1

        return pos_now

    def init_p_map(self, p: parameter.Parameter):
        length_D = int(p.v_target * p.t_det)
        pos_D = self.pos_now + [length_D, length_D]
        ...
        # 根据目标类型确定概率分布模型
        if self.type == 0:  # 平均分布
            p_map_D_L[0:(p.nx + 2 * length_D), 0:(p.ny + 2 * length_D)] = 1 / (
                    (p.nx + 2 * length_D) * (2 * length_D + p.ny))
        elif self.type == 1:  # 十字平均分布
            init_pos = self.pos_now
            p_map_D_L[init_pos[0]:init_pos[0] + 2 * length_D, init_pos[1] + length_D] = np.full([1, 2 * length_D],
                                                                                                4 * length_D + 1)
            p_map_D_L[init_pos[0] + length_D, init_pos[1]:init_pos[1] + 2 * length_D] = np.full([1, 2 * length_D],
                                                                                                4 * length_D + 1)

        elif self.type == 2:
            # func_1 = lambda x, y: (1 / (2 * math.pi * p.theta2)) * np.exp(
            #     -(1 / (2 * p.theta2)) * ((x - init_pos[0]) ** 2 + (y - init_pos[1]) ** 2))
            # func_2 = lambda x, y: scipy.integrate.dblquad(func_1, x - init_pos[0] - p.lix / 2,
            #                                               x - init_pos[0] + p.lix / 2,
            #                                               lambda g: y - init_pos[1] - p.liy / 2,
            #                                               lambda h: y - init_pos[1] + p.liy / 2)
            for i in range(length_D, p.nx + length_D):
                for j in range(length_D, p.ny + length_D):
                    p_map_D_L[i, j] = self.cal_cdf(i, j, p, pos_D)
        else:
            print('type error')
            exit(0)
    ...
    def cal_cdf(self, i: int, j: int, p: parameter.Parameter, pos_D):
        func_1 = lambda x, y: (1 / (2 * math.pi * p.theta2)) * np.exp(
            -(1 / (2 * p.theta2)) * ((x - pos_D[0]) ** 2 + (y - pos_D[1]) ** 2))
        res, err = scipy.integrate.dblquad(func_1, i - pos_D[0] - p.lix / 2,
                                           i - pos_D[0] + p.lix / 2,
                                           lambda g: j - pos_D[1] - p.liy / 2,
                                           lambda h: j - pos_D[1] + p.liy / 2)
        return res
